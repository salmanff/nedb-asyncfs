...
// sf changed exists to isPresent
storage.isPresent = function(file, customFS, callback) {
  return customFS.isPresent(file, null, callback);
}
...
storage.ensureDatafileIntegrity = function (filename, customFS, callback) {
  ...
  // sf changed exists to isPresent
  storage.isPresent(filename, customFS, function (err, filenameExists) {
    // Write was successful
    if (!err && filenameExists) { return callback(null); }
    // sf added - isPresent has 'err'
    if (err) { return callback(err) }

    storage.isPresent(tempFilename, customFS, function (err, oldFilenameExists) {
      // New database
      if (err) {
        callback(err)
      } else if (!oldFilenameExists) {
